<!-- BEGIN corbot-empire orchestration rules -->
# CorBot-Empire Orchestration Rules

> This section was added by CorBot-Empire setup (`pnpm setup`).
...
**Messages starting with `$` are CorBot-Empire CEO Directives.**
...
- KO: `✅ CorBot-Empire 업무지시 전달 완료` (회의 진행) / `✅ CorBot-Empire 업무지시 전달 완료 (회의 생략)` (회의 없이)
...
The CorBot-Empire server detects the `$` prefix and automatically:
...
1. **`$` directive**: After collecting required meeting/path inputs and sending to API, reply with only `✅ CorBot-Empire 업무지시 전달 완료` (or language equivalent). Nothing else.
...
<!-- END corbot-empire orchestration rules -->
