      ...
      >
        <div className="flex items-center justify-between border-b border-slate-700 px-6 py-5">
          <h2 className="text-lg font-bold text-white">
            {t({ ko: "Create New Task", en: "Create New Task", ja: "新しいタスクを作成", zh: "创建新任务" })}
          </h2>
          <div className="flex items-center gap-2">
            ...
            >
              {`[${t({ ko: "Temp", en: "Temp", ja: "一時", zh: "临时" })}(${draftsCount})]`}
            </button>
            <button
        ...
        <form onSubmit={onSubmit} className="flex min-h-0 flex-1 flex-col">
            ...
            <div className="min-w-0 space-y-4">
              <div>
                <label className="mb-1 block text-sm font-medium text-slate-300">
                  {t({ ko: "Title", en: "Title", ja: "タイトル", zh: "标题" })} <span className="text-red-400">*</span>
                </label>
                <input
                  type="text"
                  value={title}
              ...
              <div>
                <label className="mb-1 block text-sm font-medium text-slate-300">
                  {t({ ko: "Description", en: "Description", ja: "説明", zh: "说明" })}
                </label>
                <textarea
                  value={description}
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="mb-1 block text-sm font-medium text-slate-300">
                    {t({ ko: "Department", en: "Department", ja: "部署", zh: "部门" })}
                  </label>
                  <select
                    value={departmentId}
                ...
                <div>
                  <label className="mb-1 block text-sm font-medium text-slate-300">
                    {t({ ko: "Task Type", en: "Task Type", ja: "タスク種別", zh: "任务类型" })}
                  </label>
                  <select
                    value={taskType}
          ...
          <div className="flex justify-end gap-3 border-t border-slate-700 px-6 py-4">
            ...
            >
              {submitBusy
                ? t({ ko: "Creating...", en: "Creating...", ja: "作成中...", zh: "创建中..." })
                : t({ ko: "Create Task", en: "Create Task", ja: "タスク作成", zh: "创建任务" })}
            </button>
          </div>
        </form>
      </div>
