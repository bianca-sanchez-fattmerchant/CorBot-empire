...
function readPersistedMessengerChannels(): PersistedMessengerChannels | null {
  ...
  try {
    ...
    console.warn(`[CorBot-Empire] failed to load messenger channels settings: ${String(err)}`);
    return null;
  } finally {
    try {
      db?.close();
...
function queueWake(params: { key: string; text: string; debounceMs?: number }) {
  ...
  void sendMessengerWake(params.text).catch((err) => {
    console.warn(`[CorBot-Empire] messenger notification failed (${params.key}): ${String(err)}`);
  });
}
