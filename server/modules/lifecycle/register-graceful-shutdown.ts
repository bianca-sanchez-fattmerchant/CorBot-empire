...
export function registerGracefulShutdownHandlers({
  ...
  function gracefulShutdown(signal: string): void {
    console.log(`\n[CorBot-Empire] ${signal} received. Shutting down gracefully...`);
    ...
    for (const [taskId, child] of activeProcesses) {
      console.log(`[CorBot-Empire] Stopping process for task ${taskId} (pid: ${child.pid})`);
    ...
    wss.close(() => {
      server.close(() => {
        ...
        console.log("[CorBot-Empire] Shutdown complete.");
        process.exit(0);
      });
    });

    setTimeout(() => {
      console.error("[CorBot-Empire] Forced exit after timeout.");
      process.exit(1);
    }, 5000).unref();
  }
