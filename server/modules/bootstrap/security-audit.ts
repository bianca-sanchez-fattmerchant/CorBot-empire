...
export function createSecurityAuditTools(deps: SecurityAuditDeps) {
  ...
  function loadSecurityAuditPrevHash(): string {
    try {
      ...
      console.warn(`[CorBot-Empire] security audit chain bootstrap failed: ${String(err)}`);
    }
    return "GENESIS";
  }
  ...
  function appendSecurityAuditFallbackLog(payload: unknown): boolean {
    ...
    try {
      ...
      try {
        process.stderr.write(`[CorBot-Empire] security audit fallback append failed: ${String(fallbackErr)}\n${line}`);
  ...
  function recordMessageIngressAuditOr503(
    ...
    try {
      ...
      console.error(`[CorBot-Empire] security audit unavailable: ${String(err)}`);
  ...
  async function recordAcceptedIngressAuditOrRollback(
    ...
    try {
      ...
      console.error(
        `[CorBot-Empire] rollback after audit failure failed: message_id=${messageId}, ` + `${String(rollbackErr)}`,
      );
    }
    return false;
  }
  ...
  function recordTaskCreationAudit(input: TaskCreationAuditInput): void {
    ...
    `,
      ...
      console.warn(`[CorBot-Empire] task creation audit failed: ${String(err)}`);
    }
  }
  ...
  function setTaskCreationAuditCompletion(taskId: string, completed: boolean): void {
    try {
      ...
      console.warn(`[CorBot-Empire] task creation audit completion update failed: ${String(err)}`);
    }
  }
