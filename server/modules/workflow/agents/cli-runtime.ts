...
export function createCliRuntimeTools(deps: CliRuntimeDeps) {
  ...
  function spawnCliAgent(
    ...
    const triggerTimeout = (kind: "idle" | "hard") => {
      ...
      const msg = `[CorBot-Empire] RUN TIMEOUT (${reason})`;
      safeWrite(`\n${msg}\n`);
    ...
    child.on("error", (err) => {
      ...
      console.error(`[CorBot-Empire] spawn error for ${provider} (task ${taskId}): ${err.message}`);
      safeWrite(`\n[CorBot-Empire] SPAWN ERROR: ${err.message}\n`);
      safeEnd();
