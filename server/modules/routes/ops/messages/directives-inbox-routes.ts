...
export function registerDirectiveAndInboxRoutes(
  ...
  app.post("/api/inbox", async (req, res) => {
    ...
    if (sessionRoute && !routedAgent) {
      console.warn(
        `[CorBot-Empire] inbox session route ignored: mapped agent not found (agent_id=${sessionRoute.agentId}, channel=${sessionRoute.channel}, target=${sessionRoute.targetId})`,
      );
    }
