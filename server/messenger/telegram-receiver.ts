    ...
    try {
      ...
      if (isTelegramConflictError(status.lastError)) {
        ...
        if (!conflictWarnedInPause) {
          console.warn(`[CorBot-Empire] telegram receiver paused: ${status.lastError}`);
          console.warn(
            `[CorBot-Empire] telegram receiver hint: another bot instance is polling this token. retry in ${Math.round(
        ...
        conflictWarnedInPause = false;
        console.warn(`[CorBot-Empire] telegram receiver error: ${status.lastError}`);
      }
    } finally {
      busy = false;
