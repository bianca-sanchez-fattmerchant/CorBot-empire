...
export function initializeDatabaseRuntime(): {
  ...
  if (!process.env.DB_PATH && !fs.existsSync(DEFAULT_DB_PATH) && fs.existsSync(LEGACY_DB_PATH)) {
    ...
    console.log("[CorBot-Empire] Migrated database: climpire.sqlite -> claw-empire.sqlite");
  }
  ...
  console.log(
    `[CorBot-Empire] SQLite write resilience: busy_timeout=${SQLITE_BUSY_TIMEOUT_MS}ms, ` +
  ...
  console.log(
    `[CorBot-Empire] Review guardrails: max_rounds=${REVIEW_MAX_ROUNDS}, ` +
  ...
  console.log(
    `[CorBot-Empire] In-progress watchdog: grace=${IN_PROGRESS_ORPHAN_GRACE_MS}ms, ` +
      `sweep=${IN_PROGRESS_ORPHAN_SWEEP_MS}ms`,
  );
  console.log(`[CorBot-Empire] Subtask delegation sweep: interval=${SUBTASK_DELEGATION_SWEEP_MS}ms`);
